...
document.addEventListener("DOMContentLoaded",async()=>{const pricingContainer=document.getElementById("pricing-container");pricingContainer.innerHTML=`
    ...
    `;try{const response=await fetch("/get_pricing/?limit=4");if(!response.ok)throw new Error("خطا در دریافت تعرفه‌ها");const plans=await response.json();if(!plans||plans.length===0){pricingContainer.innerHTML=`
