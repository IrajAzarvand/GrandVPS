...
pricingContainer.innerHTML=plans.map((plan,index)=>{const isPopular=index===1;return`
            <div class="pricing-card ${isPopular ? 'popular' : ''} lazy-load">
                <div class="pricing-header">
                    <div class="provider-icon-wrapper"><img src="${plan.provider_icon}" alt="${plan.provider_name}" class="provider-icon"></div><h3 class="pricing-name">${plan.name}</h3>
                    <div class="price">
                        <div class="hourly-price">${plan.hourly_price_toman.toLocaleString()} <span>تومان/ساعت</span></div>
                    </div>
                    <div class="monthly-price">${plan.monthly_price_toman.toLocaleString()} <span>تومان/ماه</span></div>
                </div>
